...
extension ZMUserSession: SyncAgentDelegate {
        ...
        WaitingGroupTask(context: syncContext) { [weak self] in
            ...
            if let qualifiedSelfClientID {
                ...
                await mlsClientManager.initializeMLSClientIfNeeded(
                    ...
                    mlsFeature: await fetchMLSFeature(),
            ...
            await performPostQuickSyncE2EIActions()
        }
    }
    
    private func resolveOneOnOneConversationsIfNeeded() async {
        guard let clientSessionComponent, !didAlreadyResolveAllOneOnOnes else { return }
        guard await isMLSFeatureEnabled() else { return }
    ...
    private func performPostQuickSyncE2EIActions() async {
        await checkExpiredCertificateRevocationLists()
        await checkE2EICertificateExpiryStatus()
    }
    
    /// Reads the MLS feature off the syncContext-backed `FeatureConfigRepository`
    /// instead of `self.mlsFeature` (which hops to the viewContext/main queue).
    private func fetchMLSFeature() async -> Feature.MLS {
        guard let repository = clientSessionComponent?.featureConfigRepository else {
            return .init()
        }
        
        do {
            let local = try await repository.fetchMLSConfig()
            return .init(status: local.status, config: local.config ?? .init())
        } catch {
            return .init()
        }
    }
    
    private func isMLSFeatureEnabled() async -> Bool {
        guard let repository = clientSessionComponent?.featureConfigRepository else {
            return false
        }
        return await repository.isFeatureEnabled(.mls)
    }
    
    func isE2EIFeatureEnabled() async -> Bool {
        guard let repository = clientSessionComponent?.featureConfigRepository else {
            return false
        }
        return await repository.isFeatureEnabled(.e2ei)
    }
    ...
    func checkE2EICertificateExpiryStatus() async {
