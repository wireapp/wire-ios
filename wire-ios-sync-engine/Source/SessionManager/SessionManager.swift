...
public final class SessionManager: NSObject, SessionManagerType {
    ...
    fileprivate func registerObservers(account: Account, session: ZMUserSession) {
        ...
        let conversationListObserver = ConversationListChangeInfo.add(observer: self, for: ConversationContainer.conversations(inUserSession: session)!, userSession: session)
        let connectionRequestObserver = ConversationListChangeInfo.add(observer: self, for: ConversationContainer.pendingConnectionConversations(inUserSession: session)!, userSession: session)
