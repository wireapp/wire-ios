...
public enum DeveloperFlag: String, CaseIterable {
    ...
    private var defaultValue: Bool {
        guard let configurationKey else {
            return false
        }
        return DeveloperFlagsDefaults.isEnabled(configurationKey)
    }
    ...
    /// The `Developer-Flags.xcconfig` variable whose value seeds this flag's default, if any.
    ///
    /// Values are mirrored into `DeveloperFlagsDefaults` at setup time (see
    /// `scripts/generate-developer-flag-defaults.sh`).
    private var configurationKey: String? {
        switch self {
        case .createLegacyBackups:
            "CREATE_LEGACY_BACKUP_ENABLED"
        case .forceDatabaseLoadingFailure:
            "FORCE_DATABASE_LOADING_FAILURE"
        case .ignoreIncomingEvents:
            "IGNORE_INCOMING_EVENTS_ENABLED"
        case .useWireAuthentication:
            "WIRE_AUTHENTICATION_ENABLED"
        default:
            nil
        }
    }
