...
final class ClientListViewController1: UIViewController,
                                UITableViewDelegate,
                                UITableViewDataSource,
                                ClientUpdateObserver,
                                ClientColorVariantProtocol,
    ...
    var leftBarButtonItem: UIBarButtonItem? {
        if self.isIPadRegular() {
            return UIBarButtonItem.createNavigationRightBarButtonItem(
                systemImage: true,
                target: self,
                action: #selector(ClientListViewController1.backPressed(_:)))
        }
    ...
    private func createTableView() {
        ...
        tableView.rowHeight = UITableView.automaticDimension
        tableView.estimatedRowHeight = 80
        tableView.register(ClientTableViewCell1.self, forCellReuseIdentifier: ClientTableViewCell1.zm_reuseIdentifier)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: ClientTableViewCell1.zm_reuseIdentifier, for: indexPath) as? ClientTableViewCell1 {
            cell.selectionStyle = .none
            cell.showDisclosureIndicator()
...
extension ClientListViewController1: ClientRemovalObserverDelegate {
    func setIsLoadingViewVisible(_ clientRemovalObserver: ClientRemovalObserver, isVisible: Bool) {
        guard removalObserver == clientRemovalObserver else {
            return
        }
...
extension ClientListViewController1: UserObserving {
