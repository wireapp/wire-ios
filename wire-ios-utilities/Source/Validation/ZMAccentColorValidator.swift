...
@objc public final class ZMAccentColorValidator: NSObject, ZMPropertyValidator {
    ...
    @discardableResult public static func validateValue(_ ioValue: inout Any?) throws -> Bool {
        ...
        if value == nil ||
            value < AccentColor.min.rawValue ||
            AccentColor.max.rawValue < value {
            let color = AccentColor(rawValue:
                AccentColor.min.rawValue +
                    Int16(arc4random_uniform(UInt32(AccentColor.max.rawValue - AccentColor.min.rawValue)))) // swiftlint:disable:this legacy_random
