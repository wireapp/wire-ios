...
class PrimaryButtonDescription: ValueSubmission {
    let title: String
    ...
    var currentButton: IconButton?
}

extension PrimaryButtonDescription: ViewDescriptor {
    func create() -> UIView {
        let button = IconButton(fontSpec: .normalSemiboldFont)
        button.layer.cornerRadius = 16
        button.layer.masksToBounds = true
        button.setTitleColor(SemanticColors.Button.textPrimaryEnabled, for: .normal)
        button.setBackgroundImageColor(SemanticColors.Button.backgroundPrimaryEnabled, for: .normal)
        button.setBackgroundImageColor(SemanticColors.Button.backgroundPrimaryHighlighted, for: .highlighted)
        button.setBackgroundImageColor(SemanticColors.Button.backgroundPrimaryDisabled, for: .disabled)
        ...
        NSLayoutConstraint.activate([
            ...
            button.leadingAnchor.constraint(equalTo: buttonContainer.leadingAnchor, constant: 31),
            button.trailingAnchor.constraint(equalTo: buttonContainer.trailingAnchor, constant: -31)
        ])
