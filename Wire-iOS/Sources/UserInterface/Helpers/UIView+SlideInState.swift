...
public extension UIView {
    func wr_animateSlideTo(direction: SlideDirection = .Down, newState: ()->()) {
        ...
        let screenshot = self.snapshotViewAfterScreenUpdates(false)
        ...
        UIView.wr_animateWithEasing(RBBEasingFunctionEaseInOutExpo, duration: 0.20, animations: {
            self.frame = screenshot.frame
            screenshot.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y - offset, self.frame.size.width, self.frame.size.height)
            }) { _ in
