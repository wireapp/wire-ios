...
extension AudioTrackView {
    override open func updateConstraints() {
        if !initialConstraintsCreated {
            ...
            [artworkImageView, playPauseButton].forEach() {$0?.translatesAutoresizingMaskIntoConstraints = false}

            artworkImageView.fitInSuperview()
            playPauseButton.fitInSuperview()
        }
