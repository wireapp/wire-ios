...
public class TestinySync {
    ...
    private static String rcTestsComment;
    ...
    private static void syncTestinyCustomFields(List<String> actualIds, String scenarioName, Set<String> normalizedTags) {
        ...
        String tags = normalizedTags.stream()
                .filter(t -> !t.startsWith(JIRA_TAG) && !containsIgnoreCase(t, RC_TAG) && !t.startsWith(KNOWN_TAG) &&
                        ...
                        !t.startsWith(CALLING_SMOKE_TAG) && !t.startsWith(BACKEND_SMOKE_TAG) &&
                        ...
                        !t.startsWith("@testservicesmoke") && !t.startsWith("@mute") && !t.startsWith("@ignoreSendingClientIdentityError") &&
        ...
        testCase.put(TAGS_PROPERTY, tags);
    ...
    public static void syncExecutedScenarioWithTestiny(String scenarioName, TestinyExecutionStatus actualTestResult,
        ...
        if (isTestinyRCRun && testinyRunId.isEmpty()) {
            try {
                testinyRunId = Optional.of(TestinyRESTWrapper.getTestRunId(projectId, testinyRunName.get()));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        if (isTestinyRCRun && testinyRunId.isPresent()) {
            try {
                if (runDescriptionSet.compareAndSet(false, true)) {
                    TestinyRESTWrapper.updateRunDescription(testinyRunId.get(), rcTestsComment);
                }
