...
public final class CallingManager {
    ...
    public synchronized void cleanup() throws Exception {
        ...
        try {
            instanceMapping.entrySet().forEach(
                    entry -> {
    ...
    private VersionedInstanceType convertTypeStringToTypeObject(
        ...
        if (instanceType.contains(CALL_BACKEND_VERSION_SEPARATOR)) {
            ...
            switch (instanceType) {
                case "chrome":
                    return new VersionedInstanceType(instanceType, CHROME_CURRENT_VERSION);
                case "firefox":
                    return new VersionedInstanceType(instanceType, FIREFOX_CURRENT_VERSION);
                case "zcall":
                case "zcall_v3":
                    return new VersionedInstanceType(instanceType, ZCALL_CURRENT_VERSION);
                default:
                    throw new IllegalArgumentException("Could not parse instance type and/or version");
            }
        }
    ...
    private String getSupportVersion(String instanceType) {
        switch (instanceType) {
            case "chrome":
                return CHROME_SUPPORT_VERSION;
            case "firefox" :
                return FIREFOX_SUPPORT_VERSION;
            case "zcall":
            case "zcall_v3":
                return ZCALL_SUPPORT_VERSION;
            default:
