    ...
    withSecrets(config: config, secrets: secrets) {
        ...
        withCredentials([
            ...
            stage('Checkout & Clean') {
                if (tablet) {
                    checkout([$class: 'GitSCM', branches: [[name: '*/${Branch}']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'zautomation'], [$class: 'SparseCheckoutPaths', sparseCheckoutPaths: [[path: 'wire-ios-automation']]], [$class: 'CheckoutOption', timeout: 30], [$class: 'CloneOption', depth: 0, noTags: true, reference: '', shallow: true, timeout: 30], [$class: 'BuildChooserSetting', buildChooser: [$class: 'DefaultBuildChooser']]], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'zautomation', url: 'git@github.com:wireapp/wire-ios.git']]])
                } else {
                    checkout([$class: 'GitSCM', branches: [[name: '*/${Branch}']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'zautomation'], [$class: 'SparseCheckoutPaths', sparseCheckoutPaths: [[path: 'wire-ios-automation']]], [$class: 'CheckoutOption', timeout: 30], [$class: 'CloneOption', depth: 0, noTags: true, reference: '', shallow: true, timeout: 30], [$class: 'BuildChooserSetting', buildChooser: [$class: 'DefaultBuildChooser']]], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'zautomation', url: 'git@github.com:wireapp/wire-ios.git']]])
                }

                echo("Installing kubectl if not already installed")
                kubeCtlSetup = readFile("${WORKSPACE}/zautomation/wire-ios-automation/common/kubectlSetup.sh")
                sh kubeCtlSetup
            ...
            lock("${Grid}") {
                stage('Download build from S3 to grid nodes') {
                    ...
                    load('zautomation/wire-ios-automation/ios/RetrieveIPA.Jenkinsfile.groovy').setIPAFromS3(env.Track, appBuildNumber, is_simulator)
                ...
                withMaven(jdk: 'JDK17', maven: 'M3', mavenOpts: '-Xmx1024m', mavenLocalRepo: '.repository', options: [junitPublisher(disabled: true), jacocoPublisher(disabled: true)]) {
                    ...
                    stage('Build common') {
                        ...
                        sh "sh $WORKSPACE/zautomation/wire-ios-automation/common/src/main/resources/install1PasswordCLIOnNode.sh"

                        echo("Get backend connections...")
                        sh "sh $WORKSPACE/zautomation/wire-ios-automation/common/src/main/resources/backendConnections.sh"
                        ...
                        sh """
mvn clean install \\
-f "$WORKSPACE/zautomation/wire-ios-automation/common/pom.xml" \\
...
"""
                    ...
                    if (tablet) {
                        stage('Build ios') {
                            sh "mvn -DskipTests=true clean install -f \"$WORKSPACE/zautomation/wire-ios-automation/ios/pom.xml\""
                        }

                        stage('Run tests') {
                            try {
                                timeout(time: 6, unit: 'HOURS') {
                                    realtimeJUnit(keepLongStdio: true, testDataPublishers: [[$class: 'JUnitFlakyTestDataPublisher']], testResults: 'zautomation/wire-ios-automation/ios-tablet/target/xml-reports/TEST*.xml') {
                                        sh """
mvn clean integration-test \\
-f "$WORKSPACE/zautomation/wire-ios-automation/ios-tablet/pom.xml" \\
-P isOnGrid \\
-DUrl='${hubUrl}' \\
...
"""
                        ...
                        stage('Run tests') {
                            try {
                                timeout(time: 6, unit: 'HOURS') {
                                    realtimeJUnit(keepLongStdio: true, testDataPublishers: [[$class: 'JUnitFlakyTestDataPublisher']], testResults: 'zautomation/wire-ios-automation/ios/target/xml-reports/TEST*.xml') {
                                        sh """
mvn clean integration-test \\
-f "$WORKSPACE/zautomation/wire-ios-automation/ios/pom.xml" \\
-P isOnGrid \\
-DUrl='${hubUrl}' \\
-Dpicklejar.parallelism='${MAX_PARALLEL}' \\
-DisSimulator=${is_simulator} \\
...
"""
            ...
            stage('Generate test results') {
                try {
                    ...
                    if (tablet) {
                        cucumber failedFeaturesNumber: -1, failedScenariosNumber: -1, failedStepsNumber: -1, fileIncludePattern: '**/target/*report*.json', jsonReportDirectory: "${WORKSPACE}/zautomation/wire-ios-automation/ios-tablet/", mergeFeaturesById: true, pendingStepsNumber: -1, skippedStepsNumber: -1, sortingMethod: 'ALPHABETICAL', undefinedStepsNumber: -1
                    } else {
                        cucumber failedFeaturesNumber: -1, failedScenariosNumber: -1, failedStepsNumber: -1, fileIncludePattern: '**/target/*report*.json', jsonReportDirectory: "${WORKSPACE}/zautomation/wire-ios-automation/ios/", mergeFeaturesById: true, pendingStepsNumber: -1, skippedStepsNumber: -1, sortingMethod: 'ALPHABETICAL', undefinedStepsNumber: -1
                    }
                } catch (e) {
                    print e
                    if (e instanceof hudson.AbortException) {
