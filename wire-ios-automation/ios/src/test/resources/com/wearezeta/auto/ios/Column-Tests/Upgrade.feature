Feature: Upgrade
  ...
  Scenario Outline: I want to update from previous version to the current one (team acc)
    ...
    And I accept alert
    When User <TeamOwner> sends 1 default message to conversation <ConversationName>
    ...
    Examples:
      | TeamOwner | TeamName   | Member1   |  Member2   | ConversationName | Picture     | DeviceName | Email      | Password        |
      | user1Name | TeamSmart  | user2Name |  user3Name | Upgrade Test     | testing.jpg | device     | user1Email | user1Password   |