...
extension EventDecoder {
    ...
    func decryptMlsMessage(
        ...
        guard let conversation = await context.perform({ ZMConversation.fetch(with: payload.id, domain: payload.qualifiedID?.domain, in: context) }) else {
            WireLogger.mls.error("failed to decrypt mls message: conversation not found in db")
            return nil
        }

        guard await context.perform({ conversation.mlsStatus == .ready }) else {
        ...
        do {
            ...
            case .proposal(let commitDelay):
                ...
                if let mlsService = mlsService, updateEvent.source == .webSocket {
                    Task {
                        do {
