...
protocol MLSEventProcessing {
    ...
    func updateConversationIfNeeded(conversation: ZMConversation, groupID: String?, context: NSManagedObjectContext) async throws
...
class MLSEventProcessor: MLSEventProcessing {
    ...
    func updateConversationIfNeeded(
        ...
        let conversationExists = try await mlsService.conversationExists(groupID: mlsGroupID)
    ...
    func wipeMLSGroup(forConversation conversation: ZMConversation, context: NSManagedObjectContext) async {
        ...
        guard let mlsGroupID = mlsGroupID else {
