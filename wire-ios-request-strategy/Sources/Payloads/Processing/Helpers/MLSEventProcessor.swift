...
public protocol MLSEventProcessing {
    ...
    ///
    /// This method will update the conversation's `mlsStatus` to `.ready` if the underlying MLS group already exists in core crypto's local storage.
    ...
    ///   - context: The sync context.
    ///
    /// This method will notify the stale key material detector about the keying material update and upload key packages if needed.
    /// It will also sync the conversation if it's missing. And if the conversation is a one to one conversation, it will be resolved.
    ///
