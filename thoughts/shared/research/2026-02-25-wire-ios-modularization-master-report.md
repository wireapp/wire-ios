...
## Context and intent
This is an independent, volunteer, architecture review produced while examining a coding agent (Codex 5.3 extra high) as a tool for researching a relatively large codebase.
...
## Table of contents
- [Current progress (facts)](#current-progress-facts)
- [What the current approach is optimizing for](#what-the-current-approach-is-optimizing-for)
- [Early risk radar (repo-wide, high leverage, to validate)](#early-risk-radar-repo-wide-high-leverage-to-validate)
  - [Top Priority](#top-priority)
    - [1) Integration interface layer is not yet consistent across features](#1-integration-interface-layer-is-not-yet-consistent-across-features)
    - [2) Routing contracts are screen-factory oriented, not flow-capability oriented](#2-routing-contracts-are-screen-factory-oriented-not-flow-capability-oriented)
    - [3) App-boundary DI/composition cost is growing at use-case granularity](#3-app-boundary-dicomposition-cost-is-growing-at-use-case-granularity)
    - [4) Tooling and module DX are underpowered for scale (metadata drift + missing automation)](#4-tooling-and-module-dx-are-underpowered-for-scale-metadata-drift--missing-automation)
  - [Mid Priority](#mid-priority)
    - [5) Internal layer leakage: WireMessagingUI -> WireMessagingData](#5-internal-layer-leakage-wiremessagingui---wiremessagingdata)
    - [6) Shared-module ownership drift (feature semantics in shared kernels)](#6-shared-module-ownership-drift-feature-semantics-in-shared-kernels)
  - [Low Priority](#low-priority)
    - [7) Assembly re-exports are widening effective public surface](#7-assembly-re-exports-are-widening-effective-public-surface)
    - [8) Data model ownership is split: strong in some modules, centralized in others](#8-data-model-ownership-is-split-strong-in-some-modules-centralized-in-others)
- [Suggested phased advise (recommended sequence)](#suggested-phased-advise-recommended-sequence)
  - [1) Lock the target architecture design first](#1-lock-the-target-architecture-design-first)
  - [2) Converge already modularized features + app root to that design](#2-converge-already-modularized-features--app-root-to-that-design)
  - [3) Add tooling/automation for smooth and bulletproof DX](#3-add-toolingautomation-for-smooth-and-bulletproof-dx)
  - [4) Decouple wide-scope shared modules using the new automation](#4-decouple-wide-scope-shared-modules-using-the-new-automation)
  - [5) Validate design completeness with one new full-scope feature](#5-validate-design-completeness-with-one-new-full-scope-feature)
  - [6) Optimize build performance once module count grows](#6-optimize-build-performance-once-module-count-grows)
  - [Suggested execution mode (coding agent support)](#suggested-execution-mode-coding-agent-support)
- [Validation request to Wire maintainers](#validation-request-to-wire-maintainers)
- [Non-goals](#non-goals)
...
- Calling integration is narrower in app (assembly-only imports in observed host files), which further shows inconsistency, but this is not pure-interface module:
...
- [Public Interface Subreport — Suggested Next Step (Detailed Example)](./2026-02-24-public-interface-subreport.md#suggested-next-step-detailed-example).

Scope note:
- this step standardizes host integration boundaries,
- it does **not** yet solve flow-capability routing design.
...
#### Potential impact if unchanged
Host apps (or other feature-modules) remain coupled to implementation-level flow assembly and lifecycle handling.

#### Early warning signal
More feature callers requiring UI/assembly knowledge to start flows.

#### Suggested next step
Introduce explicit flow-capability contracts via `FeatureInterface` boundary modules (not screen-factory assembly contracts). Based on the Authentication example:
...
Detailed implementation sketch and acceptance criteria are documented in:
- [Routing Flow Capabilities Subreport — Suggested Next Step (Detailed Example)](./2026-02-25-routing-flow-capabilities-subreport.md#suggested-next-step-detailed-example).
...
- DI standardization is intentionally not prescribed in this issue.
...
- resolve/inject feature interfaces in app consumers (i.e. `container.resolve(WireCallingFeatureInterface.self)` or `@Dependency var wireCallingFeature: WireCallingFeatureInterface`),
...
Detailed implementation sketch and acceptance criteria are documented in:
- [DI Container Subreport — Suggested Next Step (Detailed Example)](./2026-02-24-di-container-subreport.md#suggested-next-step-detailed-example).
...
Detailed implementation sketch and acceptance criteria are documented in:
- [Tooling and DX Subreport — Suggested Next Step (Detailed Example - Module Lifecycle Automation Harness)](./2026-02-25-tooling-dx-subreport.md#suggested-next-step-detailed-example-module-lifecycle-automation-harness).
...
Detailed implementation sketch and acceptance criteria are documented in:
- [WireMessaging/WireCalling Locality Subreport — Suggested Next Step (Detailed Example)](./2026-02-24-messaging-calling-locality-subreport.md#suggested-next-step-detailed-example).

Scope note:
- this step addresses internal feature layering (`UI -> Data`) and composition ownership only,
- it does **not** replace host-facing integration interface consistency,
- it does **not** replace app-boundary DI/container strategy,
...
Detailed implementation sketch and acceptance criteria are documented in:
- [Shared/Common Modules Subreport — Suggested Next Step (Detailed Example - Decouple Wide-Scope Shared Modules)](./2026-02-25-shared-modules-subreport.md#suggested-next-step-detailed-example-decouple-wide-scope-shared-modules).
...
Detailed implementation sketch and acceptance criteria are documented in:
- [Public Interface Subreport — Suggested Next Step (Detailed Example - Assembly Export Hygiene)](./2026-02-24-public-interface-subreport.md#suggested-next-step-detailed-example-assembly-export-hygiene).

Scope note:
- this step narrows transitive assembly surface only,
- it does **not** replace `FeatureInterface` host-boundary standardization,
- it does **not** replace DI/composition ownership,
- it does **not** replace flow-capability routing contracts.
...
Detailed implementation sketch and acceptance criteria are documented in:
- [Data Model Ownership Subreport — Suggested Next Step (Detailed Example - Full AccessToken Separation)](./2026-02-25-data-model-ownership-subreport.md#suggested-next-step-detailed-example-full-accesstoken-separation).
...
- Use coding agents primarily as harness engineering support (I've followed the [DEX's context engineering guide](https://www.youtube.com/watch?v=rmvDxxNubIg)):
  - targeted repo research per step using the existing subreports
