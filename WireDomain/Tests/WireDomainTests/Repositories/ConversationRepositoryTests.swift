...
class ConversationRepositoryTests: XCTestCase {
    ...
    func testDeleteConversations_For_Group_Conversations() async throws {
        // Given
        ...
        await context.perform { [context] in
            ...
            for conversation in conversations {
                XCTAssert(uuids.contains(conversation.remoteIdentifier))
                XCTAssertEqual(conversation.isDeletedRemotely, false)
            }
        }
        ...
        // When
        let qualifiedIds = Scaffolding.conversationList.found.compactMap(\.qualifiedID)
        try await sut.deleteConversations(withQualifiedIds: Set(qualifiedIds))
        ...
        await context.perform {
            ...
            XCTAssert(conversations.count > 0)
    ...
    func testRemoveSelfUserFromConversations() async throws {
        // Given
        let qualifiedUuids = Scaffolding.conversationList.found
            .compactMap(\.qualifiedID)

        await context.perform { [context] in
            // We already have conversations in the database.
            let uuids = qualifiedUuids.map(\.uuid)
            ...
            let selfUser = self.mockSelfUser()
            ...
            for conversation in conversations {
                XCTAssertEqual(conversation.isSelfAnActiveMember, true)
            }
        }
        ...
        // When
        await sut.removeSelfUserFromConversations(withQualifiedIds: Set(qualifiedUuids))
        ...
        await context.perform {
            ...
            for conversation in conversations {
                if conversation.conversationType == .group {
                    XCTAssertEqual(conversation.isSelfAnActiveMember, false)
                }
            }
        }
    }

    func testMarkConversationsAsFetched() async throws {
        // Given
        ...
        await sut.markConversationsAsFetched(qualifiedIds: Set(qualifiedUuids))
