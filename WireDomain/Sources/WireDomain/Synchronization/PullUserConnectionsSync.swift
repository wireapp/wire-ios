...
struct PullUserConnectionsSync: PullUserConnectionsSyncProtocol {
    ...
    private let store: any ConnectionsLocalStoreProtocol

    init(
        api: any ConnectionsAPI,
        store: any ConnectionsLocalStoreProtocol
    ) {
        self.api = api
        self.store = store
    }
    ...
    func pull() async throws {
        ...
        for try await connections in connectionsPager {
            for connection in connections {
                try await store.storeConnection(connection.toDomainModel())
            }
        }
    }
