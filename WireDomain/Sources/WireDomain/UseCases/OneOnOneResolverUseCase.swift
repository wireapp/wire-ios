...
public struct OneOnOneResolverUseCase: OneOnOneResolverUseCaseProtocol {
    ...
    private func resolveAllOneOnOneConversations() async throws {
        let usersIDs = try await fetchUserIdsWithOneOnOneConversation()
        
        await withTaskGroup(of: Void.self) { group in
            for userID in usersIDs {
                group.addTask {
                    do {
                        try await self.resolveOneOnOneConversation(with: userID)
                    } catch {
    ...
    private func fetchUserIdsWithOneOnOneConversation() async throws -> [WireDataModel.QualifiedID] {
            ...
            return try context
                ...
                .compactMap { user in
                    guard let userID = user.qualifiedID else {
                        WireLogger.conversation.error(
                            "missing user's qualifiedID to resolve 1-1 conversation!"
                        )
                        return nil
                    }
                    return userID
