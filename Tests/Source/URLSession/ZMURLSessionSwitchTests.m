...
- (void)setUp
{
    ...
    self.sut = [[ZMURLSessionSwitch alloc] initWithForegroundSession:self.foregroundSession backgroundSession:self.backgroundSession sessionCancelTimerClass:FakeSessionCancelTimer.class];
}
...
- (void)testThatItCreatesASessionCancelTimerWhenSwitchingToTheBackgroundSession;
{
    ...
    FakeSessionCancelTimer *timer = timers.firstObject;
    XCTAssertEqual(timer.session, self.foregroundSession);
    XCTAssertEqual(timer.timeout, ZMSessionCancelTimerDefaultTimeout);
    XCTAssertEqual(timer.startCounter, 1);
}
