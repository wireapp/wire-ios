...
class AvailabilityTests: IntegrationTest {
    ...
    func testThatAvailabilityIsBroadcastedWhenChanged() {
        ...
        XCTAssertEqual(expectedRequests, mockTransportSession.receivedRequests().map({ $0.path }))
    }
    ...
    func testThatAvailabilityIsBroadcastedToAllConnectedUsersAndTeamMembers() {
        ...
        let connectedAndTeamMemberUUIDs = [user1, user2, user3, user4].flatMap({ self.user(for: $0)?.remoteIdentifier })
        let recipientsUUIDs = message!.recipients.flatMap({ $0.user.uuid.withUnsafeBytes({ (bytes: UnsafePointer<UInt8>) in
