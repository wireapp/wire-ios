...
- (void)testThatItCallsSetVideoPreviewAndReturnValue_hasFlowManager
{
    // given
    XCTestExpectation *callExpectation = [self expectationWithDescription:@"Method called"];
    ...
    // when
    NSError *error = nil;
    BOOL result = [self.conversation.voiceChannel setVideoPreview:view error:&error];
...
- (void)testThatItCallsSetVideoPreviewAndReturnValue_noFlowManager
{
    // given
    UIView *view = [UIView new];
...
- (void)testThatItCallsSetVideoViewAndReturnValue_hasFlowManager
{
    // given
    XCTestExpectation *callExpectation = [self expectationWithDescription:@"Method called"];
    UIView *view = [UIView new];
    ...
    // when
    NSError *error = nil;
    BOOL result = [self.conversation.voiceChannel setVideoView:view forParticipant:self.conversation.connection.to error:&error];
...
- (void)testThatItCallsSetVideoViewAndReturnValue_noFlowManager
{
    // given
    UIView *view = [UIView new];
...
- (void)testThatItSetsVideoSendStatePreviewForPendingCall
{
    [self.syncMOC performGroupedBlockAndWait:^{
        ...
        self.syncOneOnOneConversation.isVideoCall = YES;
        self.syncOneOnOneConversation.isFlowActive = NO;
        self.syncOneOnOneConversation.callDeviceIsActive = YES;
        
        [self.syncOneOnOneConversation.voiceChannel updateForStateChange];
...
- (void)testThatItSetsVideoSendStateSendNoneForEndedCall
{
    [self.syncMOC performGroupedBlockAndWait:^{
        ...
        [(MockFlowManager *)[[flowManagerMock expect] andDo:^(NSInvocation *inv) {
            NOT_USED(inv);
            [callExpectationPreview fulfill];
        }] setVideoSendState:FLOWMANAGER_VIDEO_PREVIEW forConversation:OCMOCK_ANY];
        
        [(MockFlowManager *)[[flowManagerMock expect] andDo:^(NSInvocation *inv) {
            NOT_USED(inv);
            [callExpectationSendNone fulfill];
        }] setVideoSendState:FLOWMANAGER_VIDEO_SEND_NONE forConversation:OCMOCK_ANY];
