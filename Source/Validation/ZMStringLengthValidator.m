...
+ (BOOL)validateValue:(inout __autoreleasing id *)ioString mimimumStringLength:(NSUInteger)minimumLength maximumSringLength:(NSUInteger)maximumLength error:(out NSError *__autoreleasing *)outError
{
    ...
    if (maximumLength < trimmedName.length) {
        if (outError != NULL) {
            ...
            NSError *error = [[NSError alloc] initWithDomain:ZMManagedObjectValidationErrorDomain
                                                        code:ZMManagedObjectValidationErrorCodeStringTooLong
                                                    userInfo:userInfo];
            *outError = error;
        }
        return NO;
        ...
        if (outError != NULL) {
            ...
            NSDictionary *userInfo = @{NSLocalizedDescriptionKey: description};
            NSError *error = [[NSError alloc] initWithDomain:ZMManagedObjectValidationErrorDomain
                                                        code:ZMManagedObjectValidationErrorCodeStringTooShort
                                                    userInfo:userInfo];
            *outError = error;
        }
        return NO;
