...
let ZMUserClientRemoteIdentifierKey = "remoteIdentifier"
...
public class UserClient: ZMManagedObject, UserClientType {
    ...
    public override func keysTrackedForLocalModifications() -> [AnyObject]! {
        return [ZMUserClientMarkedToDeleteKey, ZMUserClientNumberOfKeysRemainingKey, ZMUserClientMissingKey]
    }
    ...
    var hasSessionWithSelfClient: Bool {
...
enum SecurityChangeType {
    ...
    func changeSecurityLevel(conversation: ZMConversation, clients: Set<UserClient>, causedBy: ZMClientMessage?) {
        switch (self) {
        case .ClientTrusted:
...
extension UserClient {
    ...
    public func addNewClientToIgnored(client: UserClient, causedBy: ZMClientMessage? = .None) {
    ...
    public func addNewClientsToIgnored(clients: Set<UserClient>, causedBy: ZMClientMessage? = .None) {
        let notSelfClients = self.addIgnoredClients(clients)
        guard notSelfClients.count > 0 else { return}
    ...
    func changeSecurityLevel(securityChangeType: SecurityChangeType, clients: Set<UserClient>, causedBy: ZMClientMessage?) {
