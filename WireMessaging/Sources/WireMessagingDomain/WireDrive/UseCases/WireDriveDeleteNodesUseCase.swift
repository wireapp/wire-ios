...
package enum WireDriveDeleteNodesError: Error {
    case serverFailedToDeleteNodes
}

/// Deletes `WireDriveNodes`s from both the server and locally cached data or moves them to the recycle bin.
package struct WireDriveDeleteNodesUseCase: WireDriveDeleteNodesUseCaseProtocol {
    ...
    package func invoke(nodeIDs: [UUID], deletePermanently: Bool) async throws {
        ...
        if try await repository.deleteNodes(nodeIDs: nodeIDs, permanently: deletePermanently) == false {
            throw WireDriveDeleteNodesError.serverFailedToDeleteNodes
        }
    }
