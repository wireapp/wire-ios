...
package enum WireDriveRestoreNodesError: Error {
    case serverFailedToRestoreNodes
}

/// Restores `WireDriveNodes`s on the server and deletes locally cached data.
/// Restoration means moving from the recycle bin back to where the node was originally before it was moved to the
/// recycle bin.
package struct WireDriveRestoreNodesUseCase: WireDriveRestoreNodesUseCaseProtocol {
    ...
    package func invoke(nodeIDs: [UUID]) async throws {
        ...
        if try await repository.restoreNodes(nodeIDs: nodeIDs) == false {
            throw WireDriveRestoreNodesError.serverFailedToRestoreNodes
        }
    }
