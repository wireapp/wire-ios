...
package struct WireDriveDraftMetadataRepository: WireDriveDraftMetadataRepositoryProtocol {

    package init() {}

    package func imageMetadata(fileURL: URL) async throws -> WireDriveDraft.Metadata? {
        guard
    ...
    package func videoMetadata(fileURL: URL) async throws -> WireDriveDraft.Metadata? {
    ...
    package func audioMetadata(fileURL: URL) async throws -> WireDriveDraft.Metadata? {
        let asset = AVAsset(url: fileURL)
        let milliseconds = try await asset.load(.duration).seconds * 1000
