...
@MainActor
package final class WireDriveLocalAssetStore: WireDriveLocalAssetStoreProtocol {

    private let updates = PassthroughSubject<(UUID, WireMessagingDomain.WireDriveLocalAsset?), Never>()
    private let contextProvider: any ManagedObjectContextProvider
    private var assets: [UUID: WireMessagingDomain.WireDriveLocalAsset] = [:]
...
private extension WireMessagingDomain.WireDriveLocalAsset {
    ...
    func hasEqualMetadata(to other: WireMessagingDomain.WireDriveLocalAsset) -> Bool {
        eTag == other.eTag
            && path == other.path
