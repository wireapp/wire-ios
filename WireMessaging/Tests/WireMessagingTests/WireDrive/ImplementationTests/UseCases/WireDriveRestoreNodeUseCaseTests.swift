...
final class WireDriveRestoreNodeUseCaseTests {
    ...
    private let sut: WireDriveRestoreNodeVersionUseCase

    init() {
        self.sut = WireDriveRestoreNodeVersionUseCase(
    ...
    func `It fails restoring a version`() async throws {
        ...
        // when
        await #expect(throws: WireDriveRestoreNodeVersionUseCase.Failure.unableToRestoreNodeVersion) {
            // then
            try await sut.invoke(nodeID: .mockID1, versionID: .mockID10)
        }
    }
