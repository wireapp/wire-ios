...
final class DeleteDraftUseCaseTests {
    ...
    @Test(
        arguments:
        [
            WireDriveUploadStatus.uploading(progress: 0.5),
            WireDriveUploadStatus.uploaded(isDraft: true),
            WireDriveUploadStatus.cancelled,
            WireDriveUploadStatus.failed(error: .fileNotFound)
        ]
    )
    func invoke_deletesDraftFromRepository(status: WireDriveUploadStatus) async throws {
        // Given
        let nodeID = UUID()
        draftsRepository.fetchDraftNodeIDCellName_MockValue = WireDriveDraft.fixture(
            nodeID: nodeID,
    ...
    func invoke_deletesFileIfNeeded(status: WireDriveUploadStatus, requiresCleanup: Bool) async throws {
        // Given
        let nodeID = UUID()
        draftsRepository.fetchDraftNodeIDCellName_MockValue = WireDriveDraft.fixture(
            nodeID: nodeID,
