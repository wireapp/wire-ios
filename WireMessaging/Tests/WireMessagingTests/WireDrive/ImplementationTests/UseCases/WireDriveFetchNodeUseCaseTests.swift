...
final class WireDriveFetchNodeUseCaseTests {

    private let repository = MockWireDriveNodesRepositoryProtocol()
    private let cache = MockWireDriveNodeCacheProtocol()
    private let sut: WireDriveFetchNodeUseCase

    init() {
        self.sut = WireDriveFetchNodeUseCase(repository: repository, cache: cache)
