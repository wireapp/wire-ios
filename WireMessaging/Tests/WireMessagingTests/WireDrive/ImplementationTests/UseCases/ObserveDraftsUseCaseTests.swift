...
struct ObserveDraftsUseCaseTests {
    ...
    func invoke() async throws {
        ...
        let (stream, continuation) = AsyncStream.makeStream(of: [WireDriveDraft].self)
        draftsRepository.draftsFor_MockValue = stream

        // When
        let output = await sut.invoke()

        let a = WireDriveDraft.fixture()
        let b = WireDriveDraft.fixture()
        let c = WireDriveDraft.fixture()
