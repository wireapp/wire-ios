...
final class ClearPublishedDraftsUseCaseTests {
    ...
    private lazy var uploadManager = WireDriveNodeUploadManager(nodesAPI: nodesAPI)
    ...
    func invoke_deletesFilesIfNeeded() async throws {
        ...
        let draftA = WireDriveDraft.fixture(
            assetURL: assetAURL,
            status: .uploaded(isDraft: false),
            requiresCleanup: false
        )
        let draftB = WireDriveDraft.fixture(
            assetURL: assetBURL,
            status: .uploaded(isDraft: false),
...
private enum Scaffolding {
    static let cancelledDraft = WireDriveDraft.fixture(status: .cancelled)
    static let uploadingDraft = WireDriveDraft.fixture(status: .uploading(progress: 0.5))
    static let uploadedDraft = WireDriveDraft.fixture(status: .uploaded(isDraft: true))
    static let publishedDraft = WireDriveDraft.fixture(status: .uploaded(isDraft: false))
    static let failedDraft = WireDriveDraft.fixture(status: .failed(error: .fileNotFound))
}
