...
final class PublishDraftsUseCaseTests {
    ...
    private lazy var draftsRepository = DraftsRepository(
        ...
        drafts: [
            "cell-1": [
                UUID(): WireDriveDraft.fixture(status: .cancelled),
                UUID(): WireDriveDraft.fixture(status: .uploaded(isDraft: true))
            ],
            "cell-2": [
                UUID(): WireDriveDraft.fixture(status: .failed(error: .fileNotFound)),
                UUID(): WireDriveDraft.fixture(status: .uploaded(isDraft: true))
            ],
            "cell-3": [
                UUID(): WireDriveDraft.fixture(status: .uploading(progress: 0.5)),
                UUID(): WireDriveDraft.fixture(status: .uploaded(isDraft: true))
            ],
            "cell-4": [
                UUID(): WireDriveDraft.fixture(status: .uploaded(isDraft: false)),
                UUID(): WireDriveDraft.fixture(status: .uploaded(isDraft: true))
            ],
            "cell-5": [:] // Empty cell
        ]
    ...
    func invoke_whenNewDraftAddedConcurrently() async throws {
        ...
        nodesAPI.publishDraftNodeIDVersionID_MockMethod = { [draftsRepository] _, _ in
            ...
            drafts["cell-4"]?[UUID()] = WireDriveDraft.fixture(status: .uploading(progress: 0.5))
