...
final class WireCellsFetchNodeVersionsUseCaseTests {

    private let repository = MockWireCellsNodesRepositoryProtocol()
    private let sut: WireCellsFetchNodeVersionsUseCase

    init() {
        self.sut = WireCellsFetchNodeVersionsUseCase(
    ...
    func `It fails retrieving node versions`() async throws {
        ...
        // when
        await #expect(throws: WireCellsFetchNodeVersionsUseCase.Failure.unableToRetrieveNodeVersions) {
            // then
