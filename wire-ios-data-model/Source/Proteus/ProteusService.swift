...
public final class ProteusService: ProteusServiceInterface {
    ...
    public func establishSession(
        ...
        do {
            try await coreCryptoProvider.coreCrypto().perform { try $0.proteusSessionFromPrekey(
    ...
    public func deleteSession(id: ProteusSessionID) async throws {
        ...
        do {
            try await coreCryptoProvider.coreCrypto().perform { try $0.proteusSessionDelete(sessionId: id.rawValue) }
        } catch {
    ...
    func saveSession(id: ProteusSessionID) async throws {
        do {
            try await coreCryptoProvider.coreCrypto().perform { try $0.proteusSessionSave(sessionId: id.rawValue) }
        } catch {
            // TODO: Log error
    ...
    public func sessionExists(id: ProteusSessionID) async throws -> Bool {
        logger.info("checking if session exists")

        do {
            return try await coreCryptoProvider.coreCrypto().perform { try $0.proteusSessionExists(sessionId: id.rawValue) }
        } catch {
    ...
    public func encrypt(
        ...
        do {
            let encryptedBytes = try await coreCryptoProvider.coreCrypto().perform { try $0.proteusEncrypt(
                sessionId: id.rawValue,
    ...
    public func encryptBatched(
        ...
        do {
            let encryptedBatch = try await coreCryptoProvider.coreCrypto().perform { try $0.proteusEncryptBatched(
    ...
    public func decrypt(
        ...
        if try await sessionExists(id: id) {
            logger.info("session exists, decrypting...")

            let decryptedBytes: [Byte] = try await coreCryptoProvider.coreCrypto().perform {
                do {
    ...
    public func generatePrekey(id: UInt16) async throws -> String {
        ...
        do {
            return try await coreCryptoProvider.coreCrypto().perform { try $0.proteusNewPrekey(prekeyId: id).data.base64EncodedString() }
        } catch {
    ...
    public func lastPrekey() async throws -> String {
        ...
        do {
            return try await coreCryptoProvider.coreCrypto().perform { try $0.proteusLastResortPrekey().data.base64EncodedString() }
        } catch {
    ...
    private func generatePrekeys(_ range: CountableRange<UInt16>) async throws -> [IdPrekeyTuple] {
        ...
        for prekeyId in range {
            let prekey = try await generatePrekey(id: prekeyId)
            prekeys.append((id: prekeyId, prekey: prekey))
        }
        return prekeys
    }
    ...
    public func localFingerprint() async throws -> String {
        ...
        do {
            return try await coreCryptoProvider.coreCrypto().perform { try $0.proteusFingerprint() }
        } catch {
    ...
    public func remoteFingerprint(forSession id: ProteusSessionID) async throws -> String {
        ...
        do {
            return try await coreCryptoProvider.coreCrypto().perform { try $0.proteusFingerprintRemote(sessionId: id.rawValue) }
        } catch {
    ...
    public func fingerprint(fromPrekey prekey: String) async throws -> String {
        ...
        do {
            return try await coreCryptoProvider.coreCrypto().perform { try $0.proteusFingerprintPrekeybundle(prekey: prekeyBytes) }
        } catch {
