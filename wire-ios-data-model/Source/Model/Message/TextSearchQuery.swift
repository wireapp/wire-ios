...
public class TextSearchQuery: NSObject {
    ...
    public func execute() {
        ...
        syncMOC.performGroupedBlock { [weak self] in
            ...
            zmLog.debug("Searching for \"\(originalQuery)\", indexed: \(indexedMessageCount), not indexed: \(notIndexedMessageCount)")
    ...
    private func executeQueryForIndexedMessages(callCount: Int = 0, completion: @escaping () -> Void) {
        ...
        syncMOC.performGroupedBlock { [weak self] in
            ...
            guard let unwrappedRequest = request,
                  let matches = syncMOC.fetchOrAssert(request: unwrappedRequest) as? [ZMClientMessage] else { return completion() }
    ...
    private func executeQueryForNonIndexedMessages() {
        ...
        syncMOC.performGroupedBlock { [weak self] in
            ...
            guard let unwrappedRequest = request,
                  let messagesToIndex = syncMOC.fetchOrAssert(request: unwrappedRequest) as? [ZMClientMessage] else { return }
