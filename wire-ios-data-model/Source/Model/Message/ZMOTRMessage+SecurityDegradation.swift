...
public extension ZMOTRMessage {
    ...
    /// from any context.
    override var causedSecurityLevelDegradation: Bool {
        get {
...
public extension ZMConversation {
    ...
    /// in this user session (i.e. since the app was started. This will be kept in memory
    /// and not persisted).
    var messagesThatCausedSecurityLevelDegradation: [ZMOTRMessage] {
        guard let moc = managedObjectContext else { return [] }
        guard let messageIds = moc.messagesThatCausedSecurityLevelDegradationByConversation[objectID] else { return [] }
        return messageIds.compactMap {
            (try? moc.existingObject(with: $0)) as? ZMOTRMessage
        }
    }

    func clearMessagesThatCausedSecurityLevelDegradation() {
        guard let moc = managedObjectContext else { return }
