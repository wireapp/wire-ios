...
- (void)testThatItReturnsAllUnarchivedConversations
{
    ...
    user.connection.status = ZMConnectionStatusBlocked;
    XCTAssert([self.uiMOC saveOrRollback]);
...
- (void)testThatItRecreatesListsAndTokens
{
    ...
    ConversationListChangeObserver *obs = [[ConversationListChangeObserver alloc] initWithConversationList:(ZMConversationContainer *)list managedObjectContext:self.uiMOC];
    ZMConversation *c2;
    ...
    // refresh list and observer token
    {
        ...
        [(ZMConversationContainer*)list recreateWithAllConversations:allConversations predicate:[factory predicateForConversationsIncludingArchived]];
...
- (void)testThatItUpdatesWhenNewConversationsAreInserted
{
    ...
    // then
    ZMConversationContainer *list = [ZMConversation conversationsIncludingArchivedInContext:self.uiMOC];
    XCTAssertEqual(list.count, 3u);
    NSArray *expected = @[c1, c2, c3];
...
- (void)testThatItUpdatesWhenNewConnectionIsIgnored;
{
    ...
    user.connection = [ZMConnection insertNewObjectInManagedObjectContext:self.uiMOC];
    user.connection.status = ZMConnectionStatusPending;
    XCTAssert([self.uiMOC saveOrRollback]);
    
    // then
    ZMConversationContainer *list = [ZMConversation pendingConversationsInContext:self.uiMOC];
    XCTAssertEqual(list.count, 1u);
    ...
    user.connection.status = ZMConnectionStatusIgnored;
    XCTAssert([self.uiMOC saveOrRollback]);
...
- (void)testThatItUpdatesWhenNewConnectionIsAccepted;
{
    ...
    // then
    ZMConversationContainer *normalList = [ZMConversation conversationsExcludingArchivedInContext:self.uiMOC];
    ZMConversationContainer *pendingList = [ZMConversation pendingConversationsInContext:self.uiMOC];
...
- (void)testThatItUpdatesWhenTwoNewConnectionsAreAccepted;
{
    ...
    user1.oneOnOneConversation = conversation1;
    ...
    user2.oneOnOneConversation = conversation2;
    user2.connection = [ZMConnection insertNewObjectInManagedObjectContext:self.uiMOC];
    user2.connection.status = ZMConnectionStatusPending;
    XCTAssert([self.uiMOC saveOrRollback]);
    ...
    user1.connection.status = ZMConnectionStatusAccepted;
    conversation2.conversationType = ZMConversationTypeOneOnOne;
    user2.connection.status = ZMConnectionStatusAccepted;
    XCTAssert([self.uiMOC saveOrRollback]);
...
- (void)testThatItUpdatesWhenNewAConversationIsArchived;
{
    ...
    // then
    ZMConversationContainer *normalList = [ZMConversation conversationsExcludingArchivedInContext:self.uiMOC];
    ZMConversationContainer *archivedList = [ZMConversation archivedConversationsInContext:self.uiMOC];
    XCTAssertEqual(normalList.count, 1u);
    XCTAssertEqualObjects(normalList, @[conversation]);
...
- (void)testThatClearingConversationMovesItToClearedList
{
    ...
    ZMConversationContainer *activeList = [ZMConversation conversationsExcludingArchivedInContext:self.uiMOC];
    ZMConversationContainer *archivedList = [ZMConversation archivedConversationsInContext:self.uiMOC];
    ZMConversationContainer *clearedList = [ZMConversation clearedConversationsInContext:self.uiMOC];
...
- (void)testThatClearingConversationDoesNotClearOtherConversations
{
    ...
    XCTAssertEqualObjects([NSSet setWithArray:activeList.items], conversations);
    XCTAssertEqual(archivedList.count, 0u);
    XCTAssertEqual(clearedList.count, 0u);
