...
class DatabaseMigrationTestBase: XCTestCase {

    typealias MigrationAction = (NSManagedObjectContext) throws -> Void

    let bundle = Bundle(for: ZMManagedObject.self)
    let tmpStoreURL = URL(fileURLWithPath: "\(NSTemporaryDirectory())databasetest/")
