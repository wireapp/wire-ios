...
class MLSServiceTests: ZMConversationTestsBase, MLSServiceDelegate {
    ...
    // MARK: - Establish one to one

    func test_EstablishOneToOneGroup() async throws {
        // Given other user ID.
        let userID = QualifiedID.random()
        let groupID = MLSGroupID.random()

        // Mock fetch mls one to one.
        _ = MockActionHandler<SyncMLSOneToOneConversationAction>(
            result: .success(groupID),
            context: uiMOC.notificationContext
        )

        mockCoreCrypto.mockConversationExists = { _ in false }
        mockCoreCrypto.mockCreateConversation = { _, _, _ in }
        mockCoreCrypto.mockCommitPendingProposals = { _ in nil }
        mockMLSActionExecutor.mockCommitPendingProposals = { _ in [] }
        mockActionsProvider.claimKeyPackagesUserIDDomainExcludedSelfClientIDIn_MockValue = [
            KeyPackage(
                client: "clientID",
                domain: "local@domain.com",
                keyPackage: Data.random(byteCount: 8).base64String(),
                keyPackageRef: Data.random(byteCount: 8).base64String(),
                userID: userID.uuid
            )
        ]
        mockMLSActionExecutor.mockAddMembers = { _, _ in [] }

        // When establish group.
        let receivedGroupID = try await sut.establishOneToOneGroupIfNeeded(
            with: userID,
            in: syncMOC
        )

        // Then the group was created and user was added.
        XCTAssertEqual(receivedGroupID, groupID)
        XCTAssertEqual(mockCoreCrypto.mockCreateConversationCount, 1)
        XCTAssertEqual(mockMLSActionExecutor.mockAddMembersCount, 1)
    }

    func test_EstablishOneToOneGroup_AlreadyExists() async throws {
        // Given other user ID.
        let userID = QualifiedID.random()
        let groupID = MLSGroupID.random()
        ...
        mockCoreCrypto.mockConversationExists = { _ in true }
        ...
        // Then the group was created and user was added.
        XCTAssertEqual(receivedGroupID, groupID)
        XCTAssertEqual(mockCoreCrypto.mockCreateConversationCount, 0)
