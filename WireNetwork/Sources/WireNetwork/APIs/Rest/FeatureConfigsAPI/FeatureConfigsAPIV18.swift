...
final class FeatureConfigsAPIV18: FeatureConfigsAPIV17 {
    ...
    override func getFeatureConfigs() async throws -> [FeatureConfig] {
        ...
        return try ResponseParser()
            .success(code: .ok, type: FeatureConfigsResponseAPIV18.self)
...
struct FeatureConfigsResponseAPIV18: Decodable, ToAPIModelConvertible {
    ...
    // added in v18
    let preventAdminlessGroups: FeatureWithConfig<FeatureConfigResponse.PreventAdminlessGroupsV18>
    ...
    func toAPIModel() -> [FeatureConfig] {
        ...
        // added in v18
        let preventAdminlessGroupsConfig = PreventAdminlessGroupsFeatureConfig(
            status: preventAdminlessGroups.status.toAPIModel(),
            promotionStrategy: preventAdminlessGroups.config.promotionStrategy,
            deletionTimeout: preventAdminlessGroups.config.deletionTimeout,
            reminderTimeouts: preventAdminlessGroups.config.reminderTimeouts
        )
        featureConfigs.append(.preventAdminlessGroups(preventAdminlessGroupsConfig))
...
extension FeatureConfigResponse {
    ...
    struct PreventAdminlessGroupsV18: Decodable {
        let deletionTimeout: Int
        let deletionTimeoutDuration: String
        let promotionStrategy: String
        let reminderTimeoutDurations: [String]
