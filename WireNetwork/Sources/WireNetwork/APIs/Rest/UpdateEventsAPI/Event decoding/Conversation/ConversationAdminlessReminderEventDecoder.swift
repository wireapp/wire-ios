...
struct ConversationAdminlessReminderEventDecoder {

    func decode(
        from container: KeyedDecodingContainer<ConversationEventCodingKeys>
    ) throws -> ConversationAdminlessReminderEvent {
        ...
        return ConversationAdminlessReminderEvent(
            conversationID: conversationID.toAPIModel(),
            senderID: senderID.toAPIModel(),
            timestamp: timestamp.date,
            scheduledDeletionDate: payload.scheduledDeletionDate.date
        )
    }

    private struct Payload: Decodable {

        let scheduledDeletionDate: UTCTime

        enum CodingKeys: String, CodingKey {
            case scheduledDeletionDate = "deletion_scheduled_for"
        }
